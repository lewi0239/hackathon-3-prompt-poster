...
export async function generatePosterDataUrl(
  ...
  if (!token)
    return { data: null, error: "Missing HF_TOKEN/HUGGING_FACE_TOKEN in env" };
  ...
  try {
    ...
    return { data: { dataUrl: `data:image/png;base64,${b64}` }, error: null };
  } catch (err: any) {
    clearTimeout(killer);
    ...
    return {
      data: null,
      error: `HF fetch error: ${err?.message || "unknown"}`,
    };
  }
}
